...
| Primary buyer or user | Regulated SRE, security operations, platform reliability, and systems integrator teams. |
| Value wedge | Teams with strict telemetry boundaries still need structured incident evidence and replay-backed RCA. |
...
| First action | Open the README archived-status note and enterprise-readiness note, then inspect Replay suite remains runnable only if this older domain proof is relevant. |
...
- Optional entry: Trust-boundary diagnostic ($5k-$15k) when this archived proof exactly matches the buyer problem.
- Revival step: Restricted-environment pilot ($30k-$90k) only after the active successor and support boundary are clear.
- Longer-term path: Local incident review license ($120k+/year) if the repository is refreshed and assigned an owner.

## Pricing Frame

- Do not price the archive as a maintained product.
- Price the revived diagnostic, workshop, template, or implementation path.
- Use Diagnostic requests only as a discussion metric until fresh validation exists.
