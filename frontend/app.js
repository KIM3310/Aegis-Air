document.addEventListener('DOMContentLoaded', () => {
    ...
    const summaryPackHeadline = document.getElementById('reviewpack-headline');
    const summaryEvidence = document.getElementById('reviewpack-proof');
    const summaryPackTarget = document.getElementById('reviewpack-target');
    const summaryPackArtifacts = document.getElementById('reviewpack-artifacts');
    const summaryPackSequence = document.getElementById('reviewpack-sequence');
    const summaryPackDelivery = document.getElementById('reviewpack-delivery');
    ...
    const copySummaryBtn = document.getElementById('copy-summary-pack-btn');
    ...
    const dataFreshnessState = document.getElementById('data-freshness-state');
    const dataFreshnessNote = document.getElementById('data-freshness-note');
    const dataFreshnessGuard = document.getElementById('data-freshness-guard');
    const lensGrid = document.getElementById('lens-grid');
    const lensEvalBtn = document.getElementById('lens-eval-btn');
    ...
    const REVIEW_LENSES = {
        ...
        commander: [
            ...
            ['03 · Evidence summary', 'Use evidence routes when the incident needs a fast handoff to the next owner.'],
        ],
        recovery: [
            ['01 · Replay delta', 'Use drift and replay score to explain what improved versus what is still risky.'],
            ['02 · Delivery mode', 'Confirm whether the proof is recorded, webhook, or live-probe based.'],
    ...
    function renderLensCards() {
        if (!lensGrid) return;
        const cards = REVIEW_LENSES[currentLens] || REVIEW_LENSES.evaluation;
        ...
        `).join('');
        [lensEvalBtn, lensCommanderBtn, lensRecoveryBtn].forEach((btn) => btn?.classList.remove('active'));
        if (currentLens === 'evaluation') lensEvalBtn?.classList.add('active');
    ...
    function describeProofFreshness(run) {
        ...
        if (runtimeAgeMinutes === null) {
            return {
                ...
                note: replayAgeMinutes === null
                    ? 'Data freshness keeps replay and runtime timestamps visible before commander handoff.'
                    : 'Data freshness keeps replay and runtime timestamps visible before commander handoff. Replay evidence is loaded, but runtime proof still needs a fresh brief.',
        ...
        if (runtimeAgeMinutes <= 15) {
            return {
                ...
                note: `Data freshness keeps replay and runtime timestamps visible before commander handoff. Runtime brief: ${formatIsoStamp(runtimeStamp)}.`,
    ...
    function renderReplayFocus(run, label = 'Replay continuity') {
        ...
        if (dataFreshness) dataFreshness.textContent = buildProofFreshnessLabel(run);
        if (dataFreshnessState) dataFreshnessState.textContent = freshness.state;
        if (dataFreshnessNote) dataFreshnessNote.textContent = freshness.note;
        if (dataFreshnessGuard) dataFreshnessGuard.textContent = freshness.guard;
    ...
    function deriveReplayDriftBoardFromReplaySuite(data) {
        ...
        return {
            ...
            review_actions: [
                ...
                'Pair this with the summary pack and scorecard before claiming incident stability.',
            ],
        };
    }
    ...
    function deriveSummaryPackFromRuntime(data) {
        const reportContract = data.report_contract || {};
        const replaySummary = data.replay_summary || {};
        const targetService = data.target_service || {};
        return {
    ...
    function renderSummaryPack(data) {
        const evidenceBundle = data.evidence_bundle || {};
        ...
        summaryPackHeadline.textContent = data.headline || 'No summary pack headline available.';
        summaryEvidence.textContent = `${evidenceBundle.score_pct || 0}% / ${evidenceBundle.rubric_checks || 0} checks`;
        summaryPackTarget.textContent = `${targetBoundary.status || '--'} · ${targetBoundary.service || '--'}`;
        renderList(
            summaryPackArtifacts,
            [
        ...
        renderList(summaryPackDelivery, handoffContract.delivery_modes || ['No delivery modes available.']);
    }
    ...
    async function copyReviewPath() {
        const sequence = Array.from(summaryPackSequence.querySelectorAll('li'))
            .map((item) => item.textContent?.trim())
            .filter(Boolean);
        const text = [
            'Aegis-Air review path',
            `Headline: ${summaryPackHeadline.textContent || '-'}`,
            `Evidence summary: ${summaryEvidence.textContent || '-'}`,
            `Target boundary: ${summaryPackTarget.textContent || '-'}`,
            `Data freshness: ${dataFreshness?.textContent || '-'}`,
            '',
            'Review sequence',
    ...
    async function copyReviewRoutes() {
        const routes = Array.from(summaryPackArtifacts.querySelectorAll('li'))
            .map((item) => item.textContent?.trim())
            .filter(Boolean);
    ...
    async function copySummaryPackSummary() {
        const sequence = Array.from(summaryPackSequence.querySelectorAll('li'))
            .map((item) => item.textContent?.trim())
            .filter(Boolean);
        const artifacts = Array.from(summaryPackArtifacts.querySelectorAll('li'))
            .map((item) => item.textContent?.trim())
            .filter(Boolean);
        const text = [
            'Aegis-Air summary pack',
            `Headline: ${summaryPackHeadline.textContent || '-'}`,
            `Evidence summary: ${summaryEvidence.textContent || '-'}`,
            `Target boundary: ${summaryPackTarget.textContent || '-'}`,
            '',
        ...
        try {
            await copyTextToClipboard(text);
            flashButtonLabel(copySummaryBtn, 'Summary Pack', 'Copied');
        } catch (error) {
            console.warn('copy summary pack failed', error);
            flashButtonLabel(copySummaryBtn, 'Summary Pack', 'Copy failed');
        }
    }

    async function copyHandoffSnapshot() {
        const deliveryModes = Array.from(summaryPackDelivery.querySelectorAll('li'))
            .map((item) => item.textContent?.trim())
            .filter(Boolean);
    ...
    async function copyProofBundle() {
        ...
        try {
            await copyTextToClipboard(text);
            flashButtonLabel(copyEvidenceBtn, 'Evidence Summary', 'Copied');
        } catch (error) {
            console.warn('copy evidence summary failed', error);
            flashButtonLabel(copyEvidenceBtn, 'Evidence Summary', 'Copy failed');
        }
    }
    ...
    async function loadSummaryPack() {
        try {
            const data = await fetchJson('/api/summary-pack');
            renderSummaryPack(data);
        } catch (_) {
            if (latestRuntimeBrief) {
                renderSummaryPack(deriveSummaryPackFromRuntime(latestRuntimeBrief));
                return;
            }
            summaryPackHeadline.textContent = 'Summary pack unavailable.';
            renderList(summaryPackArtifacts, ['No review artifacts available.']);
            renderList(summaryPackSequence, ['Load /api/runtime/brief or /api/summary-pack when the engine is available.']);
            renderList(summaryPackDelivery, ['No delivery modes available.']);
            summaryEvidence.textContent = '--';
            summaryPackTarget.textContent = '--';
        }
    }

    async function loadReplaySuite() {
        replayRefreshState.innerHTML = '<span class="loading-spinner" aria-hidden="true"></span> Loading...';
        try {
    ...
    copySummaryBtn.addEventListener('click', copySummaryPackSummary);
    copyEvidenceBtn.addEventListener('click', copyProofBundle);
    ...
    lensEvalBtn?.addEventListener('click', () => { currentLens = 'evaluation'; renderLensCards(); });
    ...
    document.addEventListener('keydown', (event) => {
        ...
        if (key === '?') {
            if (reviewpackHotkeys) {
                reviewpackHotkeys.textContent = 'Keyboard: R routes · P pack · B evidence summary · X commander brief · T top replay · L load replay.';
            }
            return;
        }
        if (key === 'c') {
