...
def test_engine_health_and_meta():
    ...
    platform_compare = client.get("/api/platform-compare")
    ...
    summary_pack = client.get("/api/summary-pack")
    ...
    assert health.json()["links"]["platform_compare"] == "/api/platform-compare"
    assert health.json()["links"]["runtime_brief"] == "/api/runtime/brief"
    assert health.json()["links"]["summary_pack"] == "/api/summary-pack"
    ...
    assert "platform-compare-surface" in health.json()["capabilities"]
    ...
    assert "summary-pack-surface" in health.json()["capabilities"]
    ...
    assert "/api/platform-compare" in body["routes"]
    assert "/api/summary-pack" in body["routes"]
    ...
    assert brief["links"]["platform_compare"] == "/api/platform-compare"
    ...
    assert platform_compare.status_code == 200
    compare_body = platform_compare.json()
    assert compare_body["schema"] == "aegis-platform-compare-v1"
    assert compare_body["reviewer_fast_path"][0] == "/api/platform-compare"
    ...
    assert summary_pack.status_code == 200
    pack = summary_pack.json()
    assert pack["readiness_contract"] == "aegis-air-summary-pack-v1"
    assert pack["handoff_contract"]["schema"] == "aegis-air-incident-report-v1"
    assert "/api/summary-pack" in pack["evidence_bundle"]["review_endpoints"]
    assert "/api/offline-deployment-pack" in pack["evidence_bundle"]["review_endpoints"]
    assert "/api/platform-compare" in pack["evidence_bundle"]["review_endpoints"]
    assert pack["evidence_bundle"]["offline_deployment_contract"] == "aegis-air-offline-deployment-pack-v1"
    assert "/api/incident-command-board" in pack["evidence_bundle"]["review_endpoints"]
    assert "/api/runtime/scorecard" in pack["evidence_bundle"]["review_endpoints"]
    assert "/api/replay-drift-board" in pack["evidence_bundle"]["review_endpoints"]
    assert "/api/evals/replays/summary" in pack["evidence_bundle"]["review_endpoints"]
    ...
    assert pack["links"]["platform_compare"] == "/api/platform-compare"
    ...
    assert "/api/summary-pack" in offline_body["deployment_bundle"]["verification_routes"]
...
def test_runtime_scorecard_counts_runtime_events():
    ...
    assert body["links"]["summary_pack"] == "/api/summary-pack"
